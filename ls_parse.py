...
def get_linker_script_data(script):
    ...
    # only this pattern for now, more might follow!
    uwnknown_cmd = re.compile(r"\sPHDRS\s*{")
...
def get_region_range(region):
    ...
    if "end" in region:
        ...
        ret["annot"] = "__CPROVER_allocated_memory(%s, %d);" % (
            hex(start), size)
...
def main():
    ...
    sym_source.add_argument(
        "-i",
        "--sym-file",
        metavar="F",
        help="file of names of linker symbols")
